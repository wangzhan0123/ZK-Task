...
public class ZookeeperTest {
	...
	public void deletePath() throws Exception {
		ZooKeeper zk = new ZooKeeper("192.168.8.224:2181", 3000, null);
	...
	public void testCreateLocalTask() throws Exception {
		...
		Thread.sleep(1000);
		//任务bean
		TaskDefine taskDefine0 = new TaskDefine();
		//添加任务bean对应springBean
		taskDefine0.setTargetBean("taskObj");
		//添加任务bean对应方法名称
		taskDefine0.setTargetMethod("print");
		//添加执行任务时间参考quartz时间配置
		taskDefine0.setCronExpression("0/3 * * * * ?");
		//添加执行时间周期，执行之后等待时间
		taskDefine0.setPeriod(1);
		
		//添加执行开始时间
		//taskDefine0.setStartTime(new Date());
		//添加任务执行--------触发创建任务
		ConsoleManager.addScheduleTask(taskDefine0);
